...
namespace I18Next.Net.Plugins
{
    ...
    {
        private const string _fallbackLanguage = "en-US";
        ...
        public virtual async Task<string> TranslateAsync(string language, string defaultNamespace, string key, IDictionary<string, object> args)
        {
            ...
            if (result == null && defaultNamespace != actualNamespace)
                result = await ResolveTranslationAsync(language, defaultNamespace, key, args);

            if(result == null && language != _fallbackLanguage)
                result = await ResolveTranslationAsync(_fallbackLanguage, defaultNamespace, key, args);
